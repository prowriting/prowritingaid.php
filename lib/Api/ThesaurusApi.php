...
class ThesaurusApi
{
    ...
    {
        list($response) = $this->postWithHttpInfo($request);
        return $response;
    }
    ...
    {
        return $this->postAsyncWithHttpInfo($request)
            ->then(
                function ($response) {
                    return $response[0];
                }
    ...
    protected function postRequest($request)
    {
        ...
        if ($request === null || (is_array($request) && count($request) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $request when calling post'
            );
        }

        $resourcePath = '/api/thesaurus';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
